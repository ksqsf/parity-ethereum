...
lazy_static! {
    static ref SYSTEM_ACCOUNT_SECRET: Secret = {
        Secret::from_slice(&hexstr_to_h256_bytes(&SYSTEM_ACCOUNT_SECRET_BYTES))
            .unwrap()
    };
}
...
impl<EthTxT: EthTxTypeTrait> EthTxExtractor<EthTxT> {
    ...
    fn verify_tx_then_stream_out(
        ...
        let thread = (tx_verify_request.sender.to_low_u64_be() & 7) as usize;
...
impl TxReplayer {
    ...
    pub fn new(db_dir: &str, last_state_root: Option<&str>) -> TxReplayer {
        // FIXME adjust parameters to match Conflux
        let db = open_db(db_dir, &Default::default(), &DatabaseCompactionProfile::SSD).unwrap();
        let journal_db = ::journaldb::new(db.key_value().clone(), ::journaldb::Algorithm::Archive, ethcore_db::COL_STATE);
        let state_db = StateDB::new(journal_db, 10 * 1024 * 1024);
        let era = state_db.journal_db().latest_era().unwrap_or(0);

        let state;
        match last_state_root {
            Some(root) => {
                state = EthState::from_existing(state_db, root.parse().unwrap(), Default::default(), Default::default()).unwrap();
            }
            None => {
                state = EthState::new(state_db, Default::default(), Default::default());
            }
        }
    ...
    pub fn commit(&mut self, txs: u64, ops: u32) -> &H256 {
        ...
        unsafe {
            ...
            state.commit().unwrap();
            let (state_root, mut state_db) = state.drop();

            // update era
            let era = self.era.get();
            self.era.set(era + 1);

            // write
            let mut batch = state_db.journal_db().backing().transaction();
            ...
            // read the state just written
            std::ptr::write(&mut self.state, EthState::from_existing(
                state_db,
                state_root,
                Default::default(),
                Default::default()
            ...
            // update ops
            self.ops_counts.set(ops + ops_delta);
        }

        info!("statedb mem = {} MiB, journaldb mem = {} MiB, journal size = {}",
              self.state.db().mem_used() as f64 / 1024.0 / 1024.0,
              self.state.db().journal_db().mem_used() as f64 / 1024.0 / 1024.0,
              self.state.db().journal_db().journal_size());
        self.state.root()
    }

    pub fn add_tx<'a>(
        &'a mut self, tx: RealizedEthTx, 
    )
    {
        if let Some(sender) = tx.sender {
            self.state.sub_balance(&sender, &tx.amount_wei, &mut CleanupMode::NoEmpty).unwrap();
        }
        if let Some(receiver) = tx.receiver {
            self.state.add_balance(&receiver, &tx.amount_wei, CleanupMode::NoEmpty).unwrap();
        }
        ...
        if self.tx_counts.get() % Self::EPOCH_TXS == 0 {
            ...
            info!("State root committed {:?}", state_root);
        }
    }
}

fn hexstr_to_h256_bytes(hex_str: &str) -> [u8; 32] {
...
fn tx_replay(matches: ArgMatches) -> errors::Result<()> {
    let mut tx_replayer =
        TxReplayer::new(matches.value_of("storage_db_dir").unwrap(),
                        matches.value_of("last_state_root"));
    ...
    let last_state_root = tx_replayer.commit(
...
fn main() -> errors::Result<()> {
    ...
    let matches = App::new("storage benchmark")
        .arg(
...
use error_chain::*;
use ethcore::{
    engines::ethash::EthashParams, 
};
use state_db::StateDB;
use parity_ethereum::db::open_db;
use ethcore::client::DatabaseCompactionProfile;
use ethcore_types::engines::params::CommonParams as EthCommonParams;
use account_state::{State as EthState, state::CleanupMode};
use ethcore_types::{
